 ...
 * ±×·¡ÇÁ°¡ ÀÖ°í, À¯ÇâÀÌ¶ó¸é ÀÌ¸¦ ÇÑ ÁÙ·Î ³ª¿­ÇÏµÇ, sorting ÇÏ°Ô ³ª¿­À» ÇØ¾ßÇÔ. 
 */

// À¯Çâ ±×·¡ÇÁ¸¦ »ó¼Ó ¹ŞÀ½. 
public class AGraphInTopological extends DGraphInList {

	// ±âº»ÀûÀÎ IterationÀ» È°¿ëÇÑ ÄÚµå
	public void TPSort1() {
		String[] result = new String[Vertices.size()];
		int nOfVertices = Vertices.size();
		for (int i = 0; i < nOfVertices; i++) {
			result[i] = getNextNode(); // ³¡¿¡¼­ ºÎÅÍ Áö¿öÁÜ, ÁøÀÔ¸¸ ÀÖ°í ÁøÃâÀÌ ¾ø´Â °ÍÀ» Ã£¾Æ¼­ ³Ö°í »èÁ¦. Áï, ºÎ¸ğ´Â ÀÖ°í ÀÚ½ÄÀº ¾ø´Â ¸¶Áö¸·°ú °°Àº Á¸Àç
		...
		for (int i = 0; i < Vertices.size(); i++) {
			...
			for (int j = 0; j < Vertices.size(); j++) {
				for (int k = 0; k < adjacentList.get(j).size(); k++) {
					...
					// ÇöÀç vertex ¸®½ºÆ®ÀÇ °ª°ú °¢ vertex ³»ÀÇ ÀÎÁ¢ ÇÑ ³ëµåµéÀÇ °ªÀÌ °°´Ù¸é +1ÇÑ´Ù.
					// + 1ÀÌ ¾ø´Ù¸é ÀÚ½ÄÀ¸·Î °®°í ÀÖ´Â ¿¬°áµÈ ÁøÃâ ³ëµå°¡ ¾ø´Â °ÍÀÌ´Ù. 
				}
			}
	...
	//getNextNodeÀ» »ç¿ëÇÏÁö ¾Ê°í¼­µµ ÁøÀÔ¸¸ÀÖ°í ÁøÃâÀº ¾ø´Â ³ëµå¸¦ È®ÀÎ ÇÒ ¼ö ÀÖ´Ù. -> recursion
	private LinkedList<String> dfsTS(boolean[] visited, String s, LinkedList<String> R) {
		visited[Vertices.indexOf(s)] = true; // ¹æ¹®À» Çß´Ù!
		for (String x : adjacent(s)) {
			if (visited[Vertices.indexOf(x)] == false) // ÀÎÁ¢ÇÑ ³ëµåµéÀÌ ¹æ¹®À» ÇÏÁö ¾ÊÀº °æ¿ì recursion
				dfsTS(visited, x, R);
		}

		System.out.println(s + " is added a the first");
		R.addFirst(s); // recursionÀÇ ¸¶Áö¸· ºÎºĞÀ¸·Î ÀÎ½ÄÇÑ ¿ø¼Ò¸¦ ³Ö¾îÁÜ. ÁøÀÔÀº ÀÖ´Âµ¥, ÁøÃâÀº ¾÷´Â ³ëµå¸¦ ±¸ÇßÀ¸¹Ç·Î ÀÌ¿¡ ´ëÇØ print¸¦ ÇØÁÖ°í recursionÀÌ±â ¶§¹®¿¡ 
		// ±× ¾Õ¿¡ ÀÖ´Â ³ëµåµéÀ» print ÇØÁÜ
		return R;
	}
